...
def balanced(m):
    ...
    if is_weight(m):
        return True
    force = (length(left(m)) * total_weight(end(left(m)))) == (length(right(m)) * total_weight(end(right(m))))
    return force and balanced(end(left(m))) and balanced(end(right(m)))
...
def totals_tree(m):
    ...
    total_branches.append(totals_tree(end(left(m))))
    total_branches.append(totals_tree(end(right(m))))
    return tree(total_weight(m), total_branches)
...
def make_counter():
    ...
    """
    total = {}
    def counter(str_given):
        total[str_given] = 1 + total.get(str_given, 0)
        return total[str_given]
    return counter
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    amount = withdraw(0, old_password)
    if type(amount) == str:
        return amount

    def next(balance, another_password):
        if new_password == another_password:
            return withdraw(balance, old_password)
        return withdraw(balance, another_password)
    return next
...
def generate_paths(t, x):
    ...
    [[0, 2], [0, 2, 1, 2]]
    """
    def generate(t, x, total=[]):
        if label(t) == x:
            yield total + [label(t)]
        for b in branches(t):
            yield from generate(b, x, total + [label(t)])
    return generate(t, x)
