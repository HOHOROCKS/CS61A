...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    closest_rests = lambda rest: find_closest(restaurant_location(rest), centroids)
    return group_by_first([[closest_rests(rest), rest] for rest in restaurants])
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(clusters[x]) for x in range (0, len(clusters))]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    Sxx = Sxy = Syy = 0
    xResids = [x - mean(xs) for x in xs]
    yResids = [y - mean(ys) for y in ys]
    for elem in xResids:
        Sxx += elem **2
    for elem in yResids:
        Syy += elem **2
    for i in range (len(xResids)):
        Sxy += xResids[i] * yResids[i]
    b = Sxy / Sxx
    a = mean(ys) - b* mean(xs)
...
def rate_all(user, restaurants, feature_fns):
    ...
    for elem in restaurants:
        name = restaurant_name(elem)
        if elem in reviewed:
            d[name] = user_rating(user, name)
        else:
            d[name] = predictor(elem)
    return d
    # END Question 9
