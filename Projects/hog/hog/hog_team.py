...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    score1= abs(player_score//10%10-player_score%10)
    score2= abs(opponent_score//10%10-opponent_score%10)
    if score1==score2:
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        player1=strategy0(score0,score1)
        player2=strategy1(score1,score0)
        if player==0:
            score0+=take_turn(player1,score1,dice)
        ...
        if player==1:
            score1+=take_turn(player2,score0,dice)
            if is_swap(score0, score1):
                a=score1
                score1=score0
                score0=a
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    dicenum=1
    while n<=maxrolls:
        avgfunc=make_averaged(roll_dice,num_samples)
        total=avgfunc(n,dice)
        if total>avgscore:
            avgscore=total
            dicenum=n
        n+=1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score+free_bacon(opponent_score),opponent_score) and opponent_score>=score:
        return 0
    """free bacon>margin but can't trigger negative swap"""
    if free_bacon(opponent_score)>=margin:
        if opponent_score>=score or not is_swap(score+free_bacon(opponent_score),opponent_score):
            return 0
        else:
            return num_rolls
    else:
