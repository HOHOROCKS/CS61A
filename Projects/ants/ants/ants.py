...
class Place(object):
	...
	def remove_insect(self, insect):
		...
		if insect.is_ant:
			...
			# BEGIN Problem 13
			"*** YOUR CODE HERE ***"
			if isinstance(insect, QueenAnt) and not insect.queenlist.index(insect):
				return
			# END Problem 13
...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		transition = 0
		tunnel_location = self.place
		while tunnel_location != hive:
			if len(tunnel_location.bees) and transition >= self.min_range and transition <= self.max_range:
				return random_or_none(tunnel_location.bees)
			tunnel_location = tunnel_location.entrance
			transition += 1      
		# END Problem 3 and 4
...
class FireAnt(Ant):
	...
	def reduce_armor(self, amount):
		...
		"*** YOUR CODE HERE ***"
		lst_length = len(self.place.bees)
		self.armor -= amount
		index = 0
		num_attacks = 0
		if len(self.place.bees) == 0:
			self.place.bees = []
		if self.armor <= 0:
			while num_attacks < lst_length:
				self.place.bees[index].reduce_armor(self.damage)
				if lst_length == len(self.place.bees):
					index += 1              
				num_attacks += 1
			self.place.remove_insect(self)
...
class HungryAnt(Ant):
	...
	def __init__(self, armor=1, digesting = 0):
		# BEGIN Problem 6
		Insect.__init__(self, armor, digesting)
		# END Problem 6
...
# END Problem 13
	...
	def action(self, colony):
		...
		# BEGIN Problem 13
		"*** YOUR CODE HERE ***"
		if self.queenlist.index(self):
			Insect.reduce_armor(self, self.armor)
		else:
			location = self.place.exit
			while location:
				if location.ant not in self.doubledants and location.ant:
					if location.ant.is_container and location.ant.contained_ant:
						if location.ant.contained_ant not in self.doubledants:
					 		location.ant.contained_ant.damage *= 2
					 		self.doubledants.append(location.ant.contained_ant)
					location.ant.damage *= 2
					self.doubledants.append(location.ant)
				if location.ant and location.ant.is_container and location.ant in self.doubledants:
					if location.ant.contained_ant and location.ant.contained_ant not in self.doubledants:
						location.ant.contained_ant.damage *= 2
						self.doubledants.append(location.ant.contained_ant)
	...
	def reduce_armor(self, amount):
		...
		if not self.queenlist.index(self):
